...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("hugefile.txt getviz", |b| b.iter(|| get_visible_lines(black_box(mmap.as_bstr()), &vec!(), 10000, 10000, 0)));
    ...
    c.bench_function("dateparser generic", |b| b.iter(|| dateparser::parse_with_timezone(black_box(s), &Local) ) );
    // 67ns to parse the epoch timestamp though, dang, must be nice.
    c.bench_function("dateparser generic epoch", |b| b.iter(|| dateparser::parse_with_timezone(black_box("1687208330"), &Local) ) );
}
