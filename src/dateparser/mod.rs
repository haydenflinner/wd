...
//!
//! ```
//! use wd::dateparser::parse_with_timezone;
...
/// ```
/// use chrono::prelude::*;
/// use wd::dateparser::datetime::Parse;
/// use std::error::Error;
///
/// fn main() -> Result<(), Box<dyn Error>> {
...
/// ```
/// use chrono::prelude::*;
/// use wd::dateparser::timezone::parse;
/// use std::error::Error;
///
/// fn main() -> Result<(), Box<dyn Error>> {
...
pub fn parse(input: &str) -> Result<DateTime<Utc>> {
    Parse::new(&Local, None, None).parse(input)
}
...
pub fn parse_with<Tz2: TimeZone>(
    ...
    Parse::new(tz, Some(default_time), Some(default_day)).parse(input)
}

/*  don't feel like fixing the compile errors
TODO instead of doing that, refactor to not use a default time or day, but only return what was parsed.
#[cfg(test)]
mod tests {
    use super::*;
